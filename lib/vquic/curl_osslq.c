                      ...
                      size_t *num_processed)
{
  ...
  for(size_t i = 0; i < num_msg; ++i) {
    ...
    nwritten = Curl_conn_cf_send(cf->next, data, bm->data, bm->data_len, FALSE,
                                 &result);
                     ...
                     size_t *num_processed)
{
  ...
  /*
  // MASQUE FIX: Old approach of reading only 1 segment at a time
  nread = Curl_conn_cf_recv(cf->next, data,
                      (char *)(BIO_MSG_N(msg, stride, 0).data),
                      (size_t)(BIO_MSG_N(msg, stride, 0).data_len), &result);

  if(nread >= 0) {
    CURL_TRC_CF(data, cf, "ossl_bio_masque_read(len=%lu) -> %d, err=%d",
              (size_t)BIO_MSG_N(msg, stride, 0).data_len, (int)nread, result);
    BIO_MSG_N(msg, stride, 0).data_len = nread;
  }
  */

  nread = Curl_conn_cf_recv(cf->next, data,
                            (char *)msg, stride, &result);
  if(nread >= 0) {
    CURL_TRC_CF(data, cf, "ossl_bio_masque_read(dgrams read=%lu) err=%d",
                nread, result);
  }
