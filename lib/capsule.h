...
/**
 * Process UDP capsules from buffer into BIO_MSG structures with custom
 * consume callback
 * @param cf          Connection filter
 * @param data        Easy handle
 * @param recvbufq    Buffer queue containing capsule data
 ...
 * @param len         Size/stride of BIO_MSG structures
 * @param err         Error code output
 * @param consume_cb  Callback for consuming data (can be NULL)
 * @param consume_ctx Context for consume callback
 * @param stream_id   Stream ID for consume callback
 * @return Number of messages processed, or -1 on error
 */
