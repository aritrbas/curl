...
connect_sub_chain:
  ...
  if(ctx->state < CF_SETUP_CNNCT_EYEBALLS) {
    ...
    if(cf->conn->bits.udp_tunnel_proxy)
      result = cf_he_insert_after(cf, data, ctx->remotehost, TRNSPRT_TCP);
    else
  ...
  /* ABASU FIX ME - Adding Curl_cf_quic_insert_after() because
    now we need the next filter to be QUIC/HTTP/3 (which has SSL)*/
  if(cf->conn->bits.udp_tunnel_proxy) {
