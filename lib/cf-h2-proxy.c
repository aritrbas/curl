...
static void h2_consume_data(void *userdata, int32_t stream_id, size_t consumed)
{
  struct cf_h2_proxy_ctx *ctx = userdata;
  nghttp2_session_consume(ctx->h2, stream_id, consumed);
}
                                   ...
                                   char *buf, size_t len, CURLcode *err)
{
  ...
  return curl_capsule_process_udp_ex(cf, data, &ctx->tunnel.recvbuf,
                                     buf, len, err,
                                     h2_consume_data, ctx,
