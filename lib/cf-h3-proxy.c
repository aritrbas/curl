...
static void cf_ngtcp2_ctx_free(struct cf_ngtcp2_ctx *ctx)
{
  if(ctx && ctx->initialized) {
    ...
    Curl_safefree(ctx->tunnel_inbuf);
    ctx->tunnel_inbuf_len = 0;
  }
  curlx_free(ctx);
}
                              ...
                              void *stream_user_data)
{
  ...
  if(proxy_ctx->tunnel.stream == stream)
                              ...
                              void *stream_user_data)
{
  ...
  if(proxy_ctx->tunnel.stream &&
     (proxy_ctx->tunnel.stream->id == stream_id))
    proxy_ctx->tunnel.stream = NULL;
  if(rv && rv != NGTCP2_ERR_STREAM_NOT_FOUND) {
    return NGHTTP3_ERR_CALLBACK_FAILURE;
  }
                                                 ...
                                                 struct pkt_io_ctx *pktx)
{
  ...
  if(cf->next->cft == &Curl_cft_udp) {
    ...
    unsigned char *buf;
    size_t max_udp_payload = NGTCP2_MAX_UDP_PAYLOAD_SIZE;
    size_t nread;
    struct sockaddr_storage remote_addr;
                             ...
                             struct Curl_easy *data)
{
  ...
  if(proxy_ctx->tunnel.stream &&
     (proxy_ctx->tunnel.stream->id == proxy_ctx->tunnel.stream_id)) {
...
static CURLcode cf_h3_proxy_shutdown(struct Curl_cfilter *cf,
                                     struct Curl_easy *data, bool *done)
{
  return cf_ngtcp2_shutdown(cf, data, done);
}
