...
/* MASQUE FIX: The following values of #define are copied from
                cf-h2-proxy.c and curl_osslq.c
                (1) DO WE NEED ALL? HOW ARE THEY USED?
                (2) WHAT SHOULD BE THE VALUES? */
...
struct tunnel_stream
{
  ...
  BIT(closed);
  /* MASQUE FIX: Where do I use these
  BIT(reset);
  */
};
...
struct cf_h3_proxy_ctx
{
  ...
  BIT(partial_read);
  /* MASQUE FIX: Where and how to use these ? */
  /*
                                ...
                                struct Curl_easy *data)
{
  ...
  infof(data, "[handle_buffered_data.......] "
              "stream ID %zd, buflen=%zu", s->id, buflen);
  infof(data, "nwritten = %zd, buflen = %zu", nwritten, buflen);
    ...
    */
    ...
    if(!Curl_bufq_is_empty(&s->recvbuf)) {
      ...
      while(Curl_bufq_peek(&s->recvbuf, &buf, &blen)) {
        ...
        if(proxy_ctx->partial_read) {
          /* MASQUE FIX: Remove this later */
          infof(data, "[ARITRA TEST] buffer read pending %zd bytes",
          ...
          if(stream->split_data) {
            ...
            stream->split_data = FALSE;
            /* MASQUE FIX: Remove this later */
            infof(data, "[ARITRA TEST] Reset stream->recvbuf, pending data "
                        "in recvbuf = %zd ", Curl_bufq_len(&s->recvbuf));
            break;
          }
          ...
          total_recv_len += blen;
          /* MASQUE FIX: Remove this later */
          infof(data, "[ARITRA TEST] Clear recvbuf by %zu bytes, pending "
                      "data in stream recvbuf %zd ", blen,
                                ...
                                struct Curl_easy *data)
{
  ...
  for(;;) {
    ...
    if(eos && !s->send_blocked && !eos_written) {
      ...
      infof(data, "[%" FMT_PRId64 "] closing QUIC stream", s->id);
...
static ssize_t cf_h3_proxy_send(struct Curl_cfilter *cf,
                                struct Curl_easy *data,
                                ...
                                char *buf, size_t len, CURLcode *err)
{
  ...
  *err = CURLE_OK;
  ...
  if(nread < 0 && !Curl_bufq_is_empty(&proxy_ctx->inbufq)) {
    nread = Curl_bufq_read(&proxy_ctx->inbufq,
                           (unsigned char *)buf, len, err);
    if(nread < 0) {
      CURL_TRC_CF(data, cf, "[%" FMT_PRId64 "] read recvbuf(len=%zu) "
...
out:
  if(proxy_h3_progress_egress(cf, data)) {
    *err = CURLE_SEND_ERROR;
    nread = -1;
  }
  else {
...
static void proxy_h3_submit(int32_t *pstream_id,
                            struct Curl_cfilter *cf,
                            struct Curl_easy *data,
                            struct httpreq *req,
                            CURLcode *err)
...
static void cf_h3_proxy_adjust_pollset(struct Curl_cfilter *cf,
                                       struct Curl_easy *data,
                    ...
                    struct tunnel_stream *ts)
{
  ...
  if(cf->conn->bits.udp_tunnel_proxy) {
    if(ts->resp->status == 200) {
      infof(data, "[ARITRA TEST] CONNECT-UDP Response 200 OK");
      capsule_protocol = Curl_dynhds_cget(&ts->resp->headers,
                                          "capsule-protocol");
      if(capsule_protocol) {
        infof(data, "[ARITRA TEST] CONNECT-UDP Response Capsule-protocol");
  ...
  *done = FALSE;
  ...
  if(!proxy_ctx->osslq_ctx->got_first_byte) {
    ...
    if(readable > 0 && (readable & CURL_CSELECT_IN)) {
      ...
      proxy_ctx->osslq_ctx->first_byte_at = Curl_now();
    }
  }
  ...
  if(err == 1) {
    ...
    CURL_TRC_CF(data, cf, "handshake complete after %dms",
                (int)Curl_timediff(now, proxy_ctx->osslq_ctx->started_at));
...
out:
  ...
  if(result) {
    if(cf->next->cft == &Curl_cft_udp) {
      ...
      infof(data, "QUIC connect failed, issue with filter chain, "
...
#endif /* defined(USE_NGHTTP3) && !defined(CURL_DISABLE_PROXY) */