                                     ...
                                     void *user_data)
{
  ...
  s->id = (int64_t)SSL_get_stream_id(s->ssl);
...
#ifdef USE_OPENSSL_QUIC
  if(ctx->osslq_ctx)
    return &ctx->osslq_ctx->call_data;
#endif /* USE_OPENSSL_QUIC */
#ifdef USE_NGTCP2
  if(ctx->ngtcp2_ctx)
                                           ...
                                           struct Curl_easy *data)
{
  ...
  if(h3->remote_ctrl_n >= ARRAYSIZE(h3->remote_ctrl)) {
    ...
    CURL_TRC_CF(data, cf, "[%" PRId64 "] rejecting remote stream",
                stream_id);
    SSL_free(stream_ssl);
    return CURLE_FAILED_INIT;
  }
  ...
  default:
    CURL_TRC_CF(data, cf, "[%" PRId64 "] reject remote non-uni-read"
                          " stream",
                stream_id);
    SSL_free(stream_ssl);
    return CURLE_FAILED_INIT;
...
#ifdef USE_OPENSSL_QUIC
    if(proxy_ctx->osslq_ctx) {
      size_t left = buflen - nwritten;
      proxy_ctx->partial_read = TRUE;
      if((tail + left) > TMP_BUF_SIZE)
        return NGHTTP3_ERR_CALLBACK_FAILURE;
      memcpy(tmp_buf + tail, buf + nwritten, left);
      tail += left;
      return 0;
    }
#endif /* USE_OPENSSL_QUIC */
  ...
  if(rv <= 0) {
    ...
    if(detail == SSL_ERROR_WANT_READ || detail == SSL_ERROR_WANT_WRITE) {
      ...
      CURL_TRC_CF(x->data, x->cf, "[%" PRId64 "] h3_quic_recv -> RESET, "
                  "rv=%d, app_err=%" PRIu64,
                  x->s->id, rv, (uint64_t)app_error_code);
    ...
    */
    ...
    if(!Curl_bufq_is_empty(&s->recvbuf)) {
      ...
      while(Curl_bufq_peek(&s->recvbuf, &buf, &blen)) {
        ...
        CURL_TRC_CF(data, cf, "[%" PRId64 "] stream %ld, received %zd "
...
static int cb_ngtcp2_handshake_completed(ngtcp2_conn *tconn, void *user_data)
{
  ...
  if(Curl_trc_is_verbose(data)) {
    ...
    CURL_TRC_CF(data, cf, "handshake complete after %dms, remote transport["
                "max_udp_payload=%" PRIu64
                ", initial_max_data=%" PRIu64
                "]",
               (int)curlx_timediff_ms(ctx->handshake_at, ctx->started_at),
               (uint64_t)rp->max_udp_payload_size,
               (uint64_t)rp->initial_max_data);
  }
#endif
                                                 ...
                                                 struct pkt_io_ctx *pktx)
{
  ...
  if(proxy_ctx->ngtcp2_ctx) {
    ...
    if(cf->next->cft == &Curl_cft_udp) {
      ...
      unsigned char buf[65536];  /* Max UDP packet size */
      size_t nread;
      struct sockaddr_storage remote_addr;
  ...
  *done = FALSE;
  ...
  if(!ctx->shutdown_started) {
    ...
    CURL_TRC_CF(data, cf, "start shutdown(err_type=%d, err_code=%"
                ...
                (uint64_t)ctx->last_error.error_code, (int)nwritten);
  ...
  *pnwritten = -1;
  ...
  if(stream->closed) {
    ...
    if(data->conn->bits.udp_tunnel_proxy) {
      ...
      if(bufq_writable_bytes(&stream->sendbuf) < capsule_len) {
        curlx_dyn_free(&dyn);
        result = CURLE_AGAIN;
        goto out;
      }
    ...
    CURL_TRC_CF(data, cf, "[%" PRId64 "] cf_h3_proxy_send, add to "
                          "sendbuf(len=%zu) -> %zd, %d",
...
out:
  ...
  CURL_TRC_CF(data, cf, "[%" PRId64 "] cf_h3_proxy_send(len=%zu)"
                        " -> %zd, %d",
                                 ...
                                 bool eos, size_t *pnwritten)
{
  struct cf_h3_proxy_ctx *proxy_ctx = cf->ctx;
#ifdef USE_NGTCP2
  if(proxy_ctx && proxy_ctx->ngtcp2_ctx)
    return cf_h3_proxy_send_ngtcp2(cf, data, buf, len, eos, pnwritten);
#endif /* USE_NGTCP2 */
#ifdef USE_OPENSSL_QUIC
  if(proxy_ctx && proxy_ctx->osslq_ctx)
...
#endif /* USE_NGTCP2 */
  return (ssize_t)Curl_capsule_process_udp(cf, data, &proxy_ctx->inbufq, buf,
                                           len, err);
}
...
out:
  ...
  CURL_TRC_CF(data, cf, "[%" PRId64 "] cf_h3_proxy_recv(len=%zu) -> "
                        " %zd, %d",
  ...
  *input_pending = FALSE;
  ...
  if(proxy_ctx->osslq_ctx) {
    ...
    /* Added in OpenSSL v3.3.x */
    {
      ...
      CURL_TRC_CF(data, cf, "negotiated idle timeout: %" PRIu64 "ms",
                  (uint64_t)idle_ms);
                                     ...
                                     const struct Curl_easy *data)
{
  ...
  if(stream && !Curl_bufq_is_empty(&stream->recvbuf))
    return TRUE;
#endif /* USE_OPENSSL_QUIC */
#ifdef USE_NGTCP2
  if(!Curl_bufq_is_empty(&proxy_ctx->inbufq))
                                         ...
                                         struct Curl_easy *data)
{
  ...
  ctx = curlx_calloc(1, sizeof(struct cf_ngtcp2_ctx));
  if(!ctx) {
    result = CURLE_OUT_OF_MEMORY;
    goto out;
  }
  ...
  Curl_bufq_init2(&proxy_ctx->inbufq, PROXY_H3_STREAM_CHUNK_SIZE,
                  H3_TUNNEL_RECV_CHUNKS, BUFQ_OPT_SOFT_LIMIT);

  result = tunnel_stream_init(cf, &proxy_ctx->tunnel);
  if(result)
    goto out;
  ...
  /* Initialize vquic context BEFORE pktx_init which needs it */
  result = vquic_ctx_init(data, &ctx->q);
  if(result)
    goto out;

  /* Set ngtcp2_ctx in proxy_ctx BEFORE pktx_init which accesses it */
  ...
  result = Curl_vquic_tls_init(&ctx->tls, cf, data, &ctx->peer, &ALPN_SPEC_H3,
                               ...
                               &ctx->conn_ref, cf_ngtcp2_on_session_reuse);
  if(result)
    goto out;
...
#error "ngtcp2 TLS backend not defined"
  ...
  /* proxy_ctx->ngtcp2_ctx was already set earlier before pktx_init */
  proxy_ctx->partial_read = FALSE;
                                        ...
                                        struct Curl_easy *data)
{
  ...
  ctx = curlx_calloc(1, sizeof(struct cf_osslq_ctx));
  if(!ctx) {
    result = CURLE_OUT_OF_MEMORY;
    goto out;
  }
  ...
  *done = FALSE;
  ...
  if(err == 1) {
    ...
    CURL_TRC_CF(data, cf, "handshake complete after %dms",
                (int)curlx_timediff_ms(now, proxy_ctx->osslq_ctx->started_at));
                    ...
                    bool *done)
{
  ...
  *done = FALSE;
  ...
  check = Curl_timeleft_ms(data, TRUE);
  if(check <= 0) {
